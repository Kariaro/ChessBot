...
public class AlphaBetaPruningV2 implements ChessAnalyser {
	...
	private BranchResult analyseBranches(ChessBoardImpl board, ChessMove lastMove, int depth, double alpha, double beta, boolean white) {
		nodes++;
		if (depth == 0) {
			return new BranchResult(getAdvancedMaterial(board, lastMove) * (white ? 1 : -1));
		}
		...
		double bestScore = -100000 * (depth + 1);
		...
		for (Move move : moves) {
			...
			BranchResult scannedResult = analyseBranches(board, move, depth - 1, -beta, -alpha, !white);
			double score = -scannedResult.value;
			if (score > bestScore) {
			...
			// TODO: Undo move is better than updating the whole state
			state.write(board);
		}
